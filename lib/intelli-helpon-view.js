...
export default class IntelliHelponView {
  ...
  //  this.element.innerHTML = '<br/>';
    ...
    this.leave = document.createElement('span');
    this.leave.classList.add('icon-x');
    this.leave.style.float = 'right';
    this.leave.onclick = function(){
      atom.packages.loadedPackages['intelli-helpon'].mainModule.toggle();
    }
    var leaveWrap = document.createElement('a');
    leaveWrap.appendChild(this.leave);
    this.element.appendChild(leaveWrap);
  ...
  loadData(keyword, fileType){
    ...
    if(keyword.length == 0){
      ...
      outerlink.style.width = "120px";

      githubLink = document.createElement('a');
      githubLink.innerHTML = '<img src="atom://intelli-helpon/pictures/github.png" style="width:30px; height:30px;">';
      outerlink.appendChild(githubLink);
      githubLink.href = "https://github.com/search?utf8=%E2%9C%93&q=" + keyword;
      githubLink.style.marginRight = "10px";


      stackLink = document.createElement('a');
      stackLink.innerHTML = '<img src="atom://intelli-helpon/pictures/stackoverflow.png" style="width:26px; height:26px;">';
      outerlink.appendChild(stackLink);
      stackLink.href = "http://stackoverflow.com/search?q=" + keyword;
      stackLink.style.marginRight = "10px";

      docLink = document.createElement('a');
      docLink.innerHTML = '<img src="atom://intelli-helpon/pictures/doc.png" style="width:25px; height:25px;">';
      outerlink.appendChild(docLink);
      switch (fileType){
        case 'Java':
        docLink.href = "http://docs.oracle.com/apps/search/search.jsp?category=java&q="+keyword; break;
        case 'JavaScript':
        docLink.href = "https://developer.mozilla.org/en-US/search?topic=js&topic=api&topic=webdev&q=" + keyword; break;
        case 'CPP':
        docLink.href = "http://en.cppreference.com/mwiki/index.php?search="+keyword; break;
      }

      outerlink.style.float = "right";
      outerlink.style.marginBottom = "10px";
      wv.appendChild(outerlink);
